...
const EmployerDocuments = () => {
  ...
  useEffect(() => {
    const f = async () => {
      try {
        const { data } = await api.get("/api/admin/employer-documents");
  ...
  const verify = async (id) => {
    try {
      await api.put(`/api/admin/employer-documents/${id}/status`, { status: 'verified' });
  ...
  const reject = async (id) => {
    try {
      await api.put(`/api/admin/employer-documents/${id}/status`, { status: 'rejected' });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold">Employer Document Verification</h2>
